...
\paragraph{Get project - priority: critical}
Users with sufficient privileges can retrieve organization to view them, or to be used for other purposes. This use case must thus return a representation of the organisation ant the projects and users that belongs to the organisation.
\begin{itemize}
	\item Services contract:\\ \\
	The getOrganizationRequest contains the name of the Organization that must be retrieved. The getOrganizationResult contains the Organization that has been identified by the OrganizationName.
	\begin{figure}[H]
    	\centering
    	\fbox{\includegraphics[width=1.0\textwidth]{servicesContractgetOrganization}}
    	\caption{getOrganization services contract}
    	\label{fig:getOrganization_services_contract}
   	\end{figure}
\end{itemize}
